...
账号是平台的，对话是你的。<br/>
把 Claude、Claude Code（未来还有 ChatGPT、Codex）的对话记录收归本地——永久留存、全文检索、随时导出。
...
| 👥 | **多账号管理** | 自动识别导出包中的账号信息，多账号数据一库收纳、按账号筛选 |
| 📄 | **多格式导出** | 单会话 Markdown / TXT / HTML / 打印 PDF，支持全库批量导出 |
| 💾 | **全库备份与恢复** | 一键导出全库 JSON（含来源与账号），任何设备导入即恢复；也可直接备份数据库文件 |
| 📥 | **数据文件导入** | 通用消息 JSON / CSV 按联系人归组导入，字段自动识别 |
...
- [x] 中文全文检索 / 多格式导出 / 数据统计
- [x] 多账号识别与筛选
- [x] 全库 JSON 备份与恢复、数据库文件备份
- [x] 通用消息文件（JSON / CSV）导入
- [ ] ChatGPT 官方导出包导入
